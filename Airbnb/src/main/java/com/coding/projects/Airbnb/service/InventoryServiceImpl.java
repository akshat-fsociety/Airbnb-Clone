...
public class InventoryServiceImpl implements IInventoryService{
    ...
    public List<InventoryDto> getAllInventoryByRoom(Long roomId) {
        log.info("Getting all the inventory with rood id {}", roomId);
        Room room = roomRepository.findById(roomId)
                .orElseThrow(()->new ResourceNotFoundException("Room not found with ID "+roomId));
        User user = getCurrentUser();
        if(!user.equals(room.getHotel().getOwner())){
            throw new AccessDeniedException("Room does not belong to this user with id "+ user.getId());
        }
    ...
    public void updateInventory(Long roomId, UpdateInventoryRequestDto updateInventoryRequestDto) {
        log.info("updating all the inventory with room id {}", roomId);
        Room room = roomRepository.findById(roomId)
                .orElseThrow(()->new ResourceNotFoundException("Room not found with ID "+roomId));
        User user = getCurrentUser();
        if(!user.equals(room.getHotel().getOwner())){
        ...
        log.info("Locking the inventory");
